...
var _ = Describe("Hook Commands", func() {
	...
	Describe("hook sessionstart", func() {
		...
		Context("OOB /branch detection", func() {
			// Helper: write a fake transcript with forkedFrom in the first line.
			writeBranchTranscript := func(path, parentUUID string) {
				line := `{"type":"progress","forkedFrom":{"sessionId":"` + parentUUID + `","messageUuid":"msg-1"},"timestamp":"2026-01-01T00:00:00Z"}` + "\n"
				Expect(os.WriteFile(path, []byte(line), 0o644)).To(Succeed())
			}

			It("should register a /branch as a new clotilde session", func() {
				parent := session.NewSession("my-session", "parent-uuid-123")
				err := store.Create(parent)
				Expect(err).NotTo(HaveOccurred())

				transcriptFile := filepath.Join(tempDir, "branch.jsonl")
				writeBranchTranscript(transcriptFile, "parent-uuid-123")

				_ = os.Setenv("CLOTILDE_SESSION_NAME", "my-session")
				defer func() { _ = os.Unsetenv("CLOTILDE_SESSION_NAME") }()

				hookInput := map[string]string{
					"session_id":      "branch-uuid-456",
					"source":          "resume",
				...
				// Branch session should be registered
				branch, err := store.Get("my-session-branch-1")
				Expect(err).NotTo(HaveOccurred())
				Expect(branch.Metadata.SessionID).To(Equal("branch-uuid-456"))
				Expect(branch.Metadata.IsForkedSession).To(BeTrue())
				Expect(branch.Metadata.ParentSession).To(Equal("my-session"))
				Expect(branch.Metadata.TranscriptPath).To(Equal(transcriptFile))

				// Parent session should be untouched
				updated, err := store.Get("my-session")
				Expect(err).NotTo(HaveOccurred())
				Expect(updated.Metadata.SessionID).To(Equal("parent-uuid-123"))
			})

			It("should auto-increment branch name when first slot is taken", func() {
				parent := session.NewSession("my-session", "parent-uuid-999")
				err := store.Create(parent)
				Expect(err).NotTo(HaveOccurred())

				// Occupy my-session-branch-1
				existing := session.NewSession("my-session-branch-1", "some-uuid")
				err = store.Create(existing)
				Expect(err).NotTo(HaveOccurred())

				transcriptFile := filepath.Join(tempDir, "branch2.jsonl")
				writeBranchTranscript(transcriptFile, "parent-uuid-999")

				_ = os.Setenv("CLOTILDE_SESSION_NAME", "my-session")
				defer func() { _ = os.Unsetenv("CLOTILDE_SESSION_NAME") }()

				hookInput := map[string]string{
					"session_id":      "branch-uuid-777",
					"source":          "resume",
					"transcript_path": transcriptFile,
				}
				inputJSON, err := json.Marshal(hookInput)
				...
				// Should use branch-2
				branch, err := store.Get("my-session-branch-2")
				Expect(err).NotTo(HaveOccurred())
				Expect(branch.Metadata.SessionID).To(Equal("branch-uuid-777"))
			})

			It("should not register if forkedFrom does not match the parent UUID", func() {
				parent := session.NewSession("my-session", "parent-uuid-abc")
				err := store.Create(parent)
				Expect(err).NotTo(HaveOccurred())

				transcriptFile := filepath.Join(tempDir, "unrelated.jsonl")
				writeBranchTranscript(transcriptFile, "completely-different-uuid")

				_ = os.Setenv("CLOTILDE_SESSION_NAME", "my-session")
				defer func() { _ = os.Unsetenv("CLOTILDE_SESSION_NAME") }()

				hookInput := map[string]string{
					"session_id":      "branch-uuid-xyz",
					"source":          "resume",
					"transcript_path": transcriptFile,
				}
				inputJSON, err := json.Marshal(hookInput)
				...
				// No branch session should be registered
				Expect(store.Exists("my-session-branch-1")).To(BeFalse())
			})

			It("should skip branch detection when CLOTILDE_SESSION_NAME is not set", func() {
				transcriptFile := filepath.Join(tempDir, "nobranch.jsonl")
				writeBranchTranscript(transcriptFile, "some-uuid")

				hookInput := map[string]string{
					"session_id":      "branch-uuid-zzz",
					"source":          "resume",
					"transcript_path": transcriptFile,
				}
				inputJSON, err := json.Marshal(hookInput)
				...
				err = executeHookWithInput("sessionstart", inputJSON)
				Expect(err).NotTo(HaveOccurred())
				// Just verifying no panic / error; no session can be created without parent name
			})

			It("should inherit parent context into branch session", func() {
				parent := session.NewSession("ctx-parent", "ctx-parent-uuid")
				parent.Metadata.Context = "working on GH-999"
				err := store.Create(parent)
				Expect(err).NotTo(HaveOccurred())

				transcriptFile := filepath.Join(tempDir, "ctx-branch.jsonl")
				writeBranchTranscript(transcriptFile, "ctx-parent-uuid")

				_ = os.Setenv("CLOTILDE_SESSION_NAME", "ctx-parent")
				defer func() { _ = os.Unsetenv("CLOTILDE_SESSION_NAME") }()

				hookInput := map[string]string{
					"session_id":      "ctx-branch-uuid",
					"source":          "resume",
					"transcript_path": transcriptFile,
				}
				inputJSON, err := json.Marshal(hookInput)
				...
				branch, err := store.Get("ctx-parent-branch-1")
				Expect(err).NotTo(HaveOccurred())
				Expect(branch.Metadata.Context).To(Equal("working on GH-999"))
			})
		})
